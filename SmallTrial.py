...
def my_uniteigenvector_zeroeigenvalue_cluster(k):
    ...
    eigenvector_n[:,-1]=[1.0/sqrt(len(G.nodes())) for i in xrange(len(G.nodes()))] #zero eigenvalue
    for i,n in enumerate(eigenvector_n):
        eigenvector_n[i]=n/float(np.linalg.norm(n))  # normalize to unitvector
